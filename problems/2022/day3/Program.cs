...
public class Program
{
    ...
    {
        string rucksack1 = line.Substring(0, (int)(line.Count() / 2));
        string rucksack2 = line.Substring((int)(line.Count() / 2));

        //Console.WriteLine($"Rucksack: {line} split into ({rucksack1}) and ({rucksack2})");
        ...
        Console.WriteLine($"Common item: '{commonItem}' - has value {(int) commonItem}");
        if (char.IsLower(commonItem))
        {
            return ((int) commonItem) - ((int) 'a') + 1;
        }
        if (char.IsUpper(commonItem))
        {
            return ((int) commonItem) - ((int) 'A') + 27;
        }
    ...
    public static void d3p2(string path)
    {
        string[] file = File.ReadAllLines(path);

        int priority = 0;
        for (int i = 0; i < (file.Count() - 2); i += 3)
        {
            priority += ProcessCommand2(file[i], file[i + 1], file[i + 2]);
        }

        Console.WriteLine(priority);
    }
    public static int ProcessCommand2(string line1, string line2, string line3)
    {

        List<char> commonItem = new List<char>();
        foreach (char item1 in line1)
        {
            foreach (char item2 in line2)
            {
                if (item1 == item2)
                {
                    commonItem.Add(item1);
                    break;
                }
            }
        }
        ...
        char badgeItem = ' ';
        foreach (char item1 in commonItem)
        {
            foreach (char item2 in line3)
            {
                if (item1 == item2)
                {
                    badgeItem = item1;
        ...
        Console.WriteLine($"Badge item: '{badgeItem}' - has value {(int) badgeItem}");
        if (char.IsLower(badgeItem))
        {
            return ((int) badgeItem) - ((int) 'a') + 1;
        }
        if (char.IsUpper(badgeItem))
        {
            return ((int) badgeItem) - ((int) 'A') + 27;
        }
    ...
    {
        //d3("day3smallinput.txt");
        //d3("day3input.txt");

        //d3p2("day3smallinput.txt");
        d3p2("day3input.txt");
    }
}